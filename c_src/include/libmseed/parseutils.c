 ...
 *********************************************************************/
int msr_parse_selection(char *recbuf, int recbuflen, int64_t *offset, MSRecord **ppmsr, int reclen,
    ...
    if (!recbuf) return MS_GENERROR;

    if (!offset) return MS_GENERROR;

    while (*offset < recbuflen) {
        retval = msr_parse(recbuf + *offset, (int)(recbuflen - *offset), ppmsr, reclen, 0, verbose);

        if (retval) {
            if (verbose) ms_log(2, "Error parsing record at offset %" PRId64 "\n", *offset);
            ...
            if (selections && !msr_matchselect(selections, *ppmsr, NULL)) {
                *offset += (*ppmsr)->reclen;
                ...
                if (dataflag) {
                    ...
                    if (bigendianhost && (*ppmsr)->byteorder == 0) dataswapflag = 1;
...
int ms_detect(const char *record, int recbuflen) {
    ...
    while (blkt_offset != 0 && blkt_offset <= recbuflen) {
        ...
        if (blkt_type == 1000 && (int)(blkt_offset + 4 + sizeof(struct blkt_1000_s)) <= recbuflen) {
...
int ms_parse_raw(char *record, int maxreclen, flag details, flag swapflag) {
    ...
    if (details >= 1) {
        ...
        /* Print header values */
        ms_log(0, "RECORD -- %s\n", srcname);
        ms_log(0, "        sequence number: '%c%c%c%c%c%c'\n", fsdh->sequence_number[0],
               fsdh->sequence_number[1], fsdh->sequence_number[2], fsdh->sequence_number[3],
               fsdh->sequence_number[4], fsdh->sequence_number[5]);
        ms_log(0, " data quality indicator: '%c'\n", fsdh->dataquality);
        if (details > 0) ms_log(0, "               reserved: '%c'\n", fsdh->reserved);
        ms_log(0, "           station code: '%c%c%c%c%c'\n", fsdh->station[0], fsdh->station[1],
               fsdh->station[2], fsdh->station[3], fsdh->station[4]);
        ms_log(0, "            location ID: '%c%c'\n", fsdh->location[0], fsdh->location[1]);
        ms_log(0, "          channel codes: '%c%c%c'\n", fsdh->channel[0], fsdh->channel[1],
               fsdh->channel[2]);
        ms_log(0, "           network code: '%c%c'\n", fsdh->network[0], fsdh->network[1]);
        ms_log(0, "             start time: %d,%d,%d:%d:%d.%04d (unused: %d)\n", fsdh->start_time.year,
               fsdh->start_time.day, fsdh->start_time.hour, fsdh->start_time.min, fsdh->start_time.sec,
               fsdh->start_time.fract, fsdh->start_time.unused);
        ...
        if (details > 1) {
            /* Activity flags */
            b = fsdh->act_flags;
            ms_log(0, "         activity flags: [%u%u%u%u%u%u%u%u] 8 bits\n", bit(b, 0x01), bit(b, 0x02),
                   bit(b, 0x04), bit(b, 0x08), bit(b, 0x10), bit(b, 0x20), bit(b, 0x40), bit(b, 0x80));
            ...
            if (b & 0x10)
                ms_log(0,
                       "                         [Bit 4] A positive leap second happened in this record\n");
            if (b & 0x20)
                ms_log(0,
                       "                         [Bit 5] A negative leap second happened in this record\n");
    ...
    if (fsdh->blockette_offset > 46 && fsdh->blockette_offset < maxreclen) {
        ...
        while (blkt_offset != 0 && blkt_offset < maxreclen) {
            ...
            if (blkt_type == 100) {
                ...
                if (swapflag) ms_gswap4(&blkt_100->samprate);
                ...
                if (details >= 1) {
                    ...
                    if (details > 1) {
                        ...
                        ms_log(0, "             undefined flags: [%u%u%u%u%u%u%u%u] 8 bits\n", bit(b, 0x01),
                               bit(b, 0x02), bit(b, 0x04), bit(b, 0x08), bit(b, 0x10), bit(b, 0x20),
                               bit(b, 0x40), bit(b, 0x80));

                        ms_log(0, "          reserved bytes (3): %u,%u,%u\n", blkt_100->reserved[0],
                               blkt_100->reserved[1], blkt_100->reserved[2]);
                    }
                }
            }
                ...
                if (details >= 1) {
                    ...
                    ms_log(0, "           signal onset time: %d,%d,%d:%d:%d.%04d (unused: %d)\n",
                           blkt_200->time.year, blkt_200->time.day, blkt_200->time.hour, blkt_200->time.min,
                           blkt_200->time.sec, blkt_200->time.fract, blkt_200->time.unused);
                ...
                if (details >= 1) {
                    ...
                    b = blkt_201->flags;
                    ms_log(0, "       event detection flags: [%u%u%u%u%u%u%u%u] 8 bits\n", bit(b, 0x01),
                           bit(b, 0x02), bit(b, 0x04), bit(b, 0x08), bit(b, 0x10), bit(b, 0x20), bit(b, 0x40),
                           bit(b, 0x80));
                    if (b & 0x01)
                    ...
                    ms_log(0, "           signal onset time: %d,%d,%d:%d:%d.%04d (unused: %d)\n",
                           blkt_201->time.year, blkt_201->time.day, blkt_201->time.hour, blkt_201->time.min,
                           blkt_201->time.sec, blkt_201->time.fract, blkt_201->time.unused);
                    ms_log(0, "                  SNR values: ");
                ...
                if (details >= 1) {
                    ms_log(0, "      calibration start time: %d,%d,%d:%d:%d.%04d (unused: %d)\n",
                           blkt_300->time.year, blkt_300->time.day, blkt_300->time.hour, blkt_300->time.min,
                           blkt_300->time.sec, blkt_300->time.fract, blkt_300->time.unused);
                    ...
                    if (b & 0x08)
                        ms_log(0,
                               "                         [Bit 3] Calibration continued from previous "
                               "record(s)\n");
                ...
                if (details >= 1) {
                    ms_log(0, "      calibration start time: %d,%d,%d:%d:%d.%04d (unused: %d)\n",
                           blkt_310->time.year, blkt_310->time.day, blkt_310->time.hour, blkt_310->time.min,
                           blkt_310->time.sec, blkt_310->time.fract, blkt_310->time.unused);
                    if (details > 1) ms_log(0, "               reserved byte: %u\n", blkt_310->reserved1);
                ...
                if (details >= 1) {
                    ms_log(0, "      calibration start time: %d,%d,%d:%d:%d.%04d (unused: %d)\n",
                           blkt_320->time.year, blkt_320->time.day, blkt_320->time.hour, blkt_320->time.min,
                           blkt_320->time.sec, blkt_320->time.fract, blkt_320->time.unused);
                    if (details > 1) ms_log(0, "               reserved byte: %u\n", blkt_320->reserved1);
                ...
                if (details >= 1) {
                    ms_log(0, "      calibration start time: %d,%d,%d:%d:%d.%04d (unused: %d)\n",
                           blkt_390->time.year, blkt_390->time.day, blkt_390->time.hour, blkt_390->time.min,
                           blkt_390->time.sec, blkt_390->time.fract, blkt_390->time.unused);
                    if (details > 1) ms_log(0, "               reserved byte: %u\n", blkt_390->reserved1);
                ...
                if (swapflag) MS_SWAPBTIME(&blkt_395->time);

                if (details >= 1) {
                    ms_log(0, "        calibration end time: %d,%d,%d:%d:%d.%04d (unused: %d)\n",
                           blkt_395->time.year, blkt_395->time.day, blkt_395->time.hour, blkt_395->time.min,
                           blkt_395->time.sec, blkt_395->time.fract, blkt_395->time.unused);
                    if (details > 1)
                ...
                if (swapflag) ms_gswap2(&firstvalue);

                if (details >= 1) ms_log(0, "           first delay value: %u\n", firstvalue);
            }
                ...
                if (details >= 1) {
                    ...
                    ms_log(0, "           time of exception: %d,%d,%d:%d:%d.%04d (unused: %d)\n",
                           blkt_500->time.year, blkt_500->time.day, blkt_500->time.hour, blkt_500->time.min,
                           blkt_500->time.sec, blkt_500->time.fract, blkt_500->time.unused);
                    ms_log(0, "                        usec: %d\n", blkt_500->usec);
                ...
                if (details >= 1) {
                    ...
                    if (details > 1) {
                        ...
                        if (b & 0x02)
                            ms_log(0,
                                   "                         [Bit 1] 1: Blockette 2000s may NOT be "
                                   "packaged\n");
                        else
                            ms_log(0,
                                   "                         [Bit 1] 0: Blockette 2000s may be packaged\n");
                        if (!(b & 0x04) && !(b & 0x08))
                            ...
                            ms_log(0,
                                   "                      [Bits 2-3] 11: Continuation blockette in span\n");
                        else if ((b & 0x04) && !(b & 0x08))
        ...
        if (fsdh->numsamples && fsdh->data_offset <= endofblockettes) {
            ms_log(2, "%s: Data offset (%d) is within blockette chain (end of blockettes: %d)\n", srcname,
                   fsdh->data_offset, endofblockettes);
            retval++;
        }
