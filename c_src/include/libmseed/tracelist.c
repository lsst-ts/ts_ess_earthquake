    ...
    if (*ppmstl) {
        ...
        while (id) {
            ...
            while (seg) {
                ...
                if (freeprvtptr && seg->prvtptr) free(seg->prvtptr);

                /* Free data array if allocated */
                if (seg->datasamples) free(seg->datasamples);
            ...
            /* Free private pointer data if present and requested*/
            if (freeprvtptr && id->prvtptr) free(id->prvtptr);
 ...
 ***************************************************************************/
MSTraceSeg *mstl_addmsr(MSTraceList *mstl, MSRecord *msr, flag dataquality, flag autoheal, double timetol,
                        double sampratetol) {
    MSTraceID *id = 0;
    ...
    /* Add data coverage to the matching MSTraceID */
        ...
        if (lastgap <= hptimetol && lastgap >= nhptimetol && lastratecheck) {
            if (!mstl_addmsrtoseg(id->last, msr, endtime, 1)) return 0;

            seg = id->last;

            if (endtime > id->latest) id->latest = endtime;
        }
        ...
        /* Search complete segment list for matches */
            ...
            while (searchseg) {
                if (msr->starttime > searchseg->starttime) followseg = searchseg;

                whence = 0;

                postgap = msr->starttime - searchseg->endtime - hpdelta;
                if (!segbefore && postgap <= hptimetol && postgap >= nhptimetol) whence = 1;

                pregap = searchseg->starttime - endtime - hpdelta;
                if (!segafter && pregap <= hptimetol && pregap >= nhptimetol) whence = 2;
            ...
            if (segbefore) {
                ...
                if (autoheal && segafter && segbefore != segafter) {
                    ...
                    if (segafter == id->last) id->last = id->last->prev;

                    /* Remove segafter from list */
                    if (segafter->prev) segafter->prev->next = segafter->next;
                    if (segafter->next) segafter->next->prev = segafter->prev;

                    /* Free data samples, private data and segment structure */
                    if (segafter->datasamples) free(segafter->datasamples);

                    if (segafter->prvtptr) free(segafter->prvtptr);
            ...
            /* Add MSRecord coverage to new segment */
                ...
                /* Add new segment after the followseg segment */
                    ...
                    seg->prev = followseg;
                    if (followseg->next) followseg->next->prev = seg;
                    followseg->next = seg;

                    if (followseg == id->last) id->last = seg;
                }
    ...
    while (seg->next && (seg->starttime > seg->next->starttime ||
        ...
        if (seg->prev) seg->prev->next = segafter;

        if (segafter->next) segafter->next->prev = seg;
        ...
        if (id->first == seg) id->first = segafter;

        if (id->last == segafter) id->last = seg;
    }
    while (seg->prev && (seg->starttime < seg->prev->starttime ||
        ...
        if (seg->next) seg->next->prev = segbefore;

        if (segbefore->prev) segbefore->prev->next = seg;
        ...
        /* Reset first and last segment pointers if replaced */
        if (id->first == segbefore) id->first = seg;

        if (id->last == seg) id->last = segbefore;
    }
...
MSTraceSeg *mstl_addmsrtoseg(MSTraceSeg *seg, MSRecord *msr, hptime_t endtime, flag whence) {
    ...
    if (msr->datasamples && msr->numsamples > 0) {
        if (msr->sampletype != seg->sampletype) {
            ms_log(2,
                   "mstl_addmsrtoseg(): MSRecord sample type (%c) does not match segment sample type (%c)\n",
        ...
        if (!(newdatasamples = realloc(seg->datasamples,
...
MSTraceSeg *mstl_addsegtoseg(MSTraceSeg *seg1, MSTraceSeg *seg2) {
    ...
    if (seg2->datasamples && seg2->numsamples > 0) {
        ...
        if (!(newdatasamples = realloc(seg1->datasamples,
...
int mstl_convertsamples(MSTraceSeg *seg, char type, flag truncate) {
    ...
    if (seg->sampletype == type) return 0;
        ...
        if (seg->sampletype == 'f') /* Convert floats to integers with simple rounding */
        {
            for (idx = 0; idx < seg->numsamples; idx++) {
                ...
                if (!truncate && (fdata[idx] - (int32_t)fdata[idx]) > 0.000001) {
                    ms_log(1,
                           "mstl_convertsamples: Warning, loss of precision when converting floats to "
                           "integers, loss: %g\n",
        ...
        } else if (seg->sampletype == 'd') /* Convert doubles to integers with simple rounding */
        {
            for (idx = 0; idx < seg->numsamples; idx++) {
                ...
                if (!truncate && (ddata[idx] - (int32_t)ddata[idx]) > 0.000001) {
                    ms_log(1,
                           "mstl_convertsamples: Warning, loss of precision when converting doubles to "
                           "integers, loss: %g\n",
            ...
            if (!(seg->datasamples =
                          realloc(seg->datasamples, (size_t)(seg->numsamples * sizeof(int32_t))))) {
    ...
    /* Convert to 32-bit floats */
        ...
        {
            for (idx = 0; idx < seg->numsamples; idx++) fdata[idx] = (float)idata[idx];
        } else if (seg->sampletype == 'd') /* Convert doubles to floats */
        {
            for (idx = 0; idx < seg->numsamples; idx++) fdata[idx] = (float)ddata[idx];
    ...
    /* Convert to 64-bit doubles */
        ...
        {
            for (idx = 0; idx < seg->numsamples; idx++) ddata[idx] = (double)idata[idx];

            free(idata);
        } else if (seg->sampletype == 'f') /* Convert floats to doubles */
        {
            for (idx = 0; idx < seg->numsamples; idx++) ddata[idx] = (double)fdata[idx];
    ...
    while (id) {
        ...
        while (seg) {
            ...
            if (gaps > 0) {
                ...
                ms_log(0, "%-17s %-24s %-24s %-3.3g %-" PRId64 "\n", id->srcname, stime, etime, seg->samprate,
                       seg->samplecnt);
            else
...
void mstl_printsynclist(MSTraceList *mstl, char *dccid, flag subsecond) {
    ...
    while (id) {
        ...
        while (seg) {
            ...
            /* Print SYNC line */
            ms_log(0, "%s|%s|%s|%s|%s|%s||%.10g|%" PRId64 "|||||||%s\n", id->network, id->station,
                   id->location, id->channel, starttime, endtime, seg->samprate, seg->samplecnt, yearday);
    ...
    while (id) {
        ...
        while (seg->next) {
            ...
            if (gap < 0.0) {
                ...
                if ((gap * -1.0) >
                    (((double)(seg->next->endtime - seg->next->starttime) / HPTMODULUS) + delta))
