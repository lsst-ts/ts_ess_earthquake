 ...
 ***************************************************************************/
Selections *ms_matchselect(Selections *selections, char *srcname, hptime_t starttime, hptime_t endtime,
    ...
    if (ppselecttime) *ppselecttime = matchst;
...
Selections *msr_matchselect(Selections *selections, MSRecord *msr, SelectTime **ppselecttime) {
    ...
    if (!selections || !msr) return NULL;

    msr_srcname(msr, srcname, 1);
    endtime = msr_endtime(msr);

    return ms_matchselect(selections, srcname, msr->starttime, endtime, ppselecttime);
...
int ms_addselect(Selections **ppselections, char *srcname, hptime_t starttime, hptime_t endtime) {
    ...
    if (!ppselections || !srcname) return -1;
 ...
 * Return 0 on success and -1 on error.
 ***************************************************************************/
int ms_addselect_comp(Selections **ppselections, char *net, char *sta, char *loc, char *chan, char *qual,
    ...
    snprintf(srcname, sizeof(srcname), "%s_%s_%s_%s_%s", selnet, selsta, selloc, selchan, selqual);

    /* Add selection to list */
    if (ms_addselect(ppselections, srcname, starttime, endtime)) return -1;
...
int ms_readselectionsfile(Selections **ppselections, char *filename) {
    ...
    if (!ppselections || !filename) return -1;
...
static int ms_globmatch(char *string, char *pattern) {
    ...
    while (*pattern) {
        if (!*string && *pattern != '*') return GLOBMATCH_FALSE;

        switch (c = *pattern++) {
            case '*':
                while (*pattern == '*') pattern++;

                if (!*pattern) return GLOBMATCH_TRUE;

                if (*pattern != '?' && *pattern != '[' && *pattern != '\\')
                    while (*string && *pattern != *string) string++;

                while (*string) {
                    if (ms_globmatch(string, pattern)) return GLOBMATCH_TRUE;
                    string++;
                }
            ...
            case '[':
                ...
                while (!match && (c = *pattern++)) {
                    if (!*pattern) return GLOBMATCH_FALSE;
                    ...
                    if (*pattern == '-') /* c-c */
                    {
                        ...
                        if (*pattern != ']') {
                            ...
                            if (*string >= c) match = GLOBMATCH_TRUE;
                            break;
                        }
                    } else /* cc or c] */
                    {
                        if (c == *string) match = GLOBMATCH_TRUE;
                        if (*pattern != ']') {
                            if (*pattern == *string) match = GLOBMATCH_TRUE;
                        } else
                            break;
                    }
                }

                if (negate == match) return GLOBMATCH_FALSE;
            ...
            default:
                if (c != *string) return GLOBMATCH_FALSE;
                break;
        }
