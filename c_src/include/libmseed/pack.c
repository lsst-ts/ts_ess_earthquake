...
static int msr_pack_header_raw(MSRecord *msr, char *rawrec, int maxheaderlen, flag swapflag, flag normalize,
                               struct blkt_1001_s **blkt1001, char *srcname, flag verbose);
static int msr_update_header(MSRecord *msr, char *rawrec, flag swapflag, struct blkt_1001_s *blkt1001,
                             char *srcname, flag verbose);
static int msr_pack_data(void *dest, void *src, int maxsamples, int maxdatabytes, int32_t *lastintsample,
                         flag comphistory, char sampletype, flag encoding, flag swapflag, char *srcname,
                         flag verbose);
 ...
 ***************************************************************************/
int msr_pack(MSRecord *msr, void (*record_handler)(char *, int, void *), void *handlerdata,
    ...
    if (packheaderbyteorder == -2) {
        if ((envvariable = getenv("PACK_HEADER_BYTEORDER"))) {
            if (*envvariable != '0' && *envvariable != '1') {
                ...
                if (verbose > 2) ms_log(1, "PACK_HEADER_BYTEORDER=0, packing little-endian header\n");
            } else {
                packheaderbyteorder = 1;
                if (verbose > 2) ms_log(1, "PACK_HEADER_BYTEORDER=1, packing big-endian header\n");
            }
        } else {
            packheaderbyteorder = -1;
        }
    ...
    if (packdatabyteorder == -2) {
        if ((envvariable = getenv("PACK_DATA_BYTEORDER"))) {
            if (*envvariable != '0' && *envvariable != '1') {
                ...
                packdatabyteorder = 0;
                if (verbose > 2) ms_log(1, "PACK_DATA_BYTEORDER=0, packing little-endian data samples\n");
            } else {
                packdatabyteorder = 1;
                if (verbose > 2) ms_log(1, "PACK_DATA_BYTEORDER=1, packing big-endian data samples\n");
            }
        } else {
            packdatabyteorder = -1;
        }
    ...
    if (msr->sequence_number <= 0 || msr->sequence_number > 999999) msr->sequence_number = 1;
    ...
    if (msr->byteorder != ms_bigendianhost()) headerswapflag = dataswapflag = 1;
    ...
    if (!msr->Blkt1000) {
        ...
        if (verbose > 2) ms_log(1, "%s: Adding 1000 Blockette\n", srcname);
    ...
    headerlen =
            msr_pack_header_raw(msr, rawrec, msr->reclen, headerswapflag, 1, &HPblkt1001, srcname, verbose);
    ...
    if (msr->encoding == DE_STEIM1 || msr->encoding == DE_STEIM2) {
        dataoffset = 64;
        while (dataoffset < headerlen) dataoffset += 64;
    ...
    *HPnumsamples = 0;
    ...
    while ((msr->numsamples - totalpackedsamples) > maxsamples || flush) {
        packsamples = msr_pack_data(rawrec + dataoffset, (char *)msr->datasamples + packoffset,
                                    (int)(msr->numsamples - totalpackedsamples), maxdatabytes,
                                    &msr->ststate->lastintsample, msr->ststate->comphistory, msr->sampletype,
                                    msr->encoding, dataswapflag, srcname, verbose);
        ...
        *HPnumsamples = (uint16_t)packsamples;
        ...
        if (verbose > 0) ms_log(1, "%s: Packed %d samples\n", srcname, packsamples);
        ...
        if (!msr->ststate->comphistory) msr->ststate->comphistory = 1;

        if (totalpackedsamples >= msr->numsamples) break;
    }

    if (verbose > 2) ms_log(1, "%s: Packed %d total samples\n", srcname, totalpackedsamples);
...
int msr_pack_header(MSRecord *msr, flag normalize, flag verbose) {
    ...
    /* Check to see if byte swapping is needed */
    if (msr->byteorder != ms_bigendianhost()) headerswapflag = 1;
    ...
    headerlen = msr_pack_header_raw(msr, msr->record, maxheaderlen, headerswapflag, normalize, NULL, srcname,
                                    verbose);
...
static int msr_pack_header_raw(MSRecord *msr, char *rawrec, int maxheaderlen, flag swapflag, flag normalize,
    ...
    if (verbose > 2) ms_log(1, "%s: Packing fixed section of data header\n", srcname);
    ...
    if (maxheaderlen < (int)sizeof(struct fsdh_s)) {
        ms_log(2, "msr_pack_header_raw(%s): maxheaderlen of %d is too small, must be >= %d\n", srcname,
               maxheaderlen, sizeof(struct fsdh_s));
        return -1;
    }
    ...
    if (msr->sequence_number > 999999) msr->sequence_number = 1;
    ...
    while (cur_blkt && offset < maxheaderlen) {
        ...
        if (cur_blkt->blkt_type == 100) {
            ...
            if (packdatabyteorder >= 0) blkt_1000->byteorder = packdatabyteorder;
        }
            ...
            if (blkt1001) *blkt1001 = blkt_1001;
        }
    ...
    if (verbose > 2) ms_log(1, "%s: Packed %d blockettes\n", srcname, blktcnt);
...
static int msr_update_header(MSRecord *msr, char *rawrec, flag swapflag, struct blkt_1001_s *blkt1001,
    ...
    if (verbose > 2) ms_log(1, "%s: Updating fixed section of data header\n", srcname);
...
static int msr_pack_data(void *dest, void *src, int maxsamples, int maxdatabytes, int32_t *lastintsample,
    ...
    if (getenv("ENCODE_DEBUG")) encodedebug = 1;
    ...
    switch (encoding) {
        case DE_ASCII:
            ...
            if (verbose > 1) ms_log(1, "%s: Packing ASCII data\n", srcname);
        ...
        case DE_INT16:
            ...
            if (verbose > 1) ms_log(1, "%s: Packing INT16 data samples\n", srcname);
        ...
        case DE_INT32:
            ...
            if (verbose > 1) ms_log(1, "%s: Packing INT32 data samples\n", srcname);
        ...
        case DE_FLOAT32:
            ...
            if (verbose > 1) ms_log(1, "%s: Packing FLOAT32 data samples\n", srcname);
        ...
        case DE_FLOAT64:
            ...
            if (verbose > 1) ms_log(1, "%s: Packing FLOAT64 data samples\n", srcname);
        ...
        case DE_STEIM1:
            ...
            if (verbose > 1) ms_log(1, "%s: Packing Steim1 data frames\n", srcname);
            ...
            if (lastintsample && nsamples > 0) *lastintsample = intbuff[nsamples - 1];
        ...
        case DE_STEIM2:
            ...
            if (verbose > 1) ms_log(1, "%s: Packing Steim2 data frames\n", srcname);
