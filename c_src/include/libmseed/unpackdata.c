 ...
 ************************************************************************/
int msr_decode_int16(int16_t *input, int samplecount, int32_t *output, int outputlength, int swapflag) {
    int16_t sample;
    int idx;
 ...
 * Return number of samples in output buffer on success, -1 on error.
 ************************************************************************/
int msr_decode_int32(int32_t *input, int samplecount, int32_t *output, int outputlength, int swapflag) {
    int32_t sample;
    int idx;
 ...
 * Return number of samples in output buffer on success, -1 on error.
 ************************************************************************/
int msr_decode_float32(float *input, int samplecount, float *output, int outputlength, int swapflag) {
    float sample;
    int idx;
 ...
 * Return number of samples in output buffer on success, -1 on error.
 ************************************************************************/
int msr_decode_float64(double *input, int samplecount, double *output, int outputlength, int swapflag) {
    double sample;
    int idx;
 ...
 * Return number of samples in output buffer on success, -1 on error.
 ************************************************************************/
int msr_decode_steim1(int32_t *input, int inputlength, int samplecount, int32_t *output, int outputlength,
    ...
    if (!input || !output || outputlength <= 0 || maxframes <= 0) return -1;
    ...
    for (frameidx = 0; frameidx < maxframes && samplecount > 0; frameidx++) {
        ...
        if (frameidx == 0) {
            ...
            if (decodedebug) ms_log(1, "Frame %d: X0=%d  Xn=%d\n", frameidx, X0, Xn);
        } else {
            startnibble = 1; /* Subsequent frames: skip nibbles */

            if (decodedebug) ms_log(1, "Frame %d\n", frameidx);
        }

        /* Swap 32-bit word containing the nibbles */
        if (swapflag) ms_gswap4a(&frame[0]);
        ...
        for (widx = startnibble; widx < 16 && samplecount > 0; widx++) {
            ...
            switch (nibble) {
                ...
                case 3: /* 11: One 4-byte difference */
                    diffcount = 1;
                    if (swapflag) ms_gswap4a(&word->d32);

                    if (decodedebug) ms_log(1, "  W%02d: 11=1x32b  %d\n", widx, word->d32);
                    break;
 ...
 * Return number of samples in output buffer on success, -1 on error.
 ************************************************************************/
int msr_decode_steim2(int32_t *input, int inputlength, int samplecount, int32_t *output, int outputlength,
                      char *srcname, int swapflag) {
    int32_t *outputptr = output; /* Pointer to next output sample location */
    uint32_t frame[16];          /* Frame, 16 x 32-bit quantities = 64 bytes */
    int32_t X0 = 0;              /* Forward integration constant, aka first sample */
        ...
        for (widx = startnibble; widx < 16 && samplecount > 0; widx++) {
            ...
            switch (nibble) {
                ...
                case 2: /* nibble=10: Must consult dnib, the high order two bits */
                    if (swapflag) ms_gswap4a(&frame[widx]);
                    ...
                    switch (dnib) {
                        ...
                        case 1: /* nibble=10, dnib=01: One 30-bit difference */
                            ...
                            if (decodedebug) ms_log(1, "  W%02d: 10,01=1x30b  %d\n", widx, diff[0]);
                            break;
 ...
 * Return number of samples in output buffer on success, -1 on error.
 ************************************************************************/
int msr_decode_geoscope(char *input, int samplecount, float *output, int outputlength, int encoding,
                        char *srcname, int swapflag) {
    int idx = 0;
    ...
    if (samplecount <= 0 || outputlength <= 0) return -1;
    ...
    for (idx = 0; idx < samplecount && outputlength >= (int)sizeof(float); idx++) {
        switch (encoding) {
            case DE_GEOSCOPE24:
                sample32.i = 0;
                if (swapflag)
                    for (k = 0; k < 3; k++) sample32.b[2 - k] = input[k];
                else
                    for (k = 0; k < 3; k++) sample32.b[1 + k] = input[k];

                mantissa = sample32.i;

                /* Take 2's complement for mantissa for overflow */
                if (mantissa > MAX24) mantissa -= 2 * (MAX24 + 1);
 ...
 * Return number of samples in output buffer on success, -1 on error.
 ************************************************************************/
int msr_decode_cdsn(int16_t *input, int samplecount, int32_t *output, int outputlength, int swapflag) {
    int32_t idx = 0;
 ...
 * Return number of samples in output buffer on success, -1 on error.
 ************************************************************************/
int msr_decode_sro(int16_t *input, int samplecount, int32_t *output, int outputlength, char *srcname,
                   int swapflag) {
    int32_t idx = 0;
    int32_t mantissa;   /* mantissa */
    int32_t gainrange;  /* gain range factor */
 ...
 * Return number of samples in output buffer on success, -1 on error.
 ************************************************************************/
int msr_decode_dwwssn(int16_t *input, int samplecount, int32_t *output, int outputlength, int swapflag) {
    int32_t idx = 0;
    int32_t sample;
    uint16_t sint;
    ...
    for (idx = 0; idx < samplecount && outputlength >= (int)sizeof(int32_t); idx++) {
        ...
        if (sample > MAX16) sample -= 2 * (MAX16 + 1);
