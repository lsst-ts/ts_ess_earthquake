...
int main(int argc, char **argv) {
    ...
    while ((retcode = ms_readmsr(&msr, inputfile, reclen, NULL, &lastrecord, 1, 1, verbose)) == MS_NOERROR) {
        ...
        if (netcode) strncpy(msr->network, netcode, sizeof(msr->network));
        ...
        /* Pack records from a MSTraceGroup */
            ...
            if (tracepack == 1) {
                mst = mstg->traces;
                while (mst) {
                    packedrecords += mst_pack(mst, &record_handler, NULL, packreclen, packencoding, byteorder,
                                              &packedsamples, 0, verbose, (MSRecord *)mst->prvtptr);
                    mst = mst->next;
                }
            ...
            if (tracepack == 2 && lastrecord) {
                ...
                while (mst) {
                    packedrecords += mst_pack(mst, &record_handler, NULL, packreclen, packencoding, byteorder,
                                              &packedsamples, 1, verbose, (MSRecord *)mst->prvtptr);
                    mst = mst->next;
                }
    ...
    if (tracepack) {
        ...
        if (packedrecords) ms_log(1, "Packed %d records\n", packedrecords);
    }

    if (retcode != MS_ENDOFFILE) ms_log(2, "Error reading %s: %s\n", inputfile, ms_errorstr(retcode));
    ...
    if (msr->sampletype != encodingtype) {
        ...
        /* Convert to integers */
            ...
            } else if (msr->sampletype == 'd') /* Convert doubles to integers with simple rounding */
            {
                ...
                if (!(msr->datasamples =
                              realloc(msr->datasamples, (size_t)(msr->numsamples * sizeof(int32_t))))) {
        ...
        /* Convert to floats */
            ...
            {
                for (idx = 0; idx < msr->numsamples; idx++) fdata[idx] = (float)idata[idx];
            } else if (msr->sampletype == 'd') /* Convert doubles to floats */
            {
                for (idx = 0; idx < msr->numsamples; idx++) fdata[idx] = (float)ddata[idx];

                /* Reallocate buffer for reduced size needed */
                if (!(msr->datasamples =
                              realloc(msr->datasamples, (size_t)(msr->numsamples * sizeof(float))))) {
                    ms_log(2, "Error, cannot re-allocate buffer for sample conversion\n");
                    return -1;
                }
            }
        ...
        /* Convert to doubles */
            ...
            {
                for (idx = 0; idx < msr->numsamples; idx++) ddata[idx] = (double)idata[idx];

                free(idata);
            } else if (msr->sampletype == 'f') /* Convert floats to doubles */
            {
                for (idx = 0; idx < msr->numsamples; idx++) ddata[idx] = (double)fdata[idx];
 ...
 ***************************************************************************/
static void term_handler(int sig) { exit(0); }
#endif
