 ...
 ************************************************************************/
int msr_encode_text(char *input, int samplecount, char *output, int outputlength) {
    int length;
    ...
    if (outputlength > 0) memset(output + length, 0, outputlength);
 ...
 * Return number of samples in output buffer on success, -1 on failure.
 ************************************************************************/
int msr_encode_int16(int32_t *input, int samplecount, int16_t *output, int outputlength, int swapflag) {
    int idx;
    ...
    for (idx = 0; idx < samplecount && outputlength >= (int)sizeof(int16_t); idx++) {
        ...
        if (swapflag) ms_gswap2a(&output[idx]);

        outputlength -= sizeof(int16_t);
    }

    if (outputlength) memset(&output[idx], 0, outputlength);
 ...
 * Return number of samples in output buffer on success, -1 on failure.
 ************************************************************************/
int msr_encode_int32(int32_t *input, int samplecount, int32_t *output, int outputlength, int swapflag) {
    int idx;
    ...
    for (idx = 0; idx < samplecount && outputlength >= (int)sizeof(int32_t); idx++) {
        ...
        if (swapflag) ms_gswap4a(&output[idx]);
 ...
 * Return number of samples in output buffer on success, -1 on failure.
 ************************************************************************/
int msr_encode_float32(float *input, int samplecount, float *output, int outputlength, int swapflag) {
    int idx;
 ...
 * Return number of samples in output buffer on success, -1 on failure.
 ************************************************************************/
int msr_encode_float64(double *input, int samplecount, double *output, int outputlength, int swapflag) {
    int idx;
    ...
    for (idx = 0; idx < samplecount && outputlength >= (int)sizeof(double); idx++) {
        ...
        if (swapflag) ms_gswap8a(&output[idx]);
 ...
 * Return number of samples in output buffer on success, -1 on failure.
 ************************************************************************/
int msr_encode_steim1(int32_t *input, int samplecount, int32_t *output, int outputlength, int32_t diff0,
                      int swapflag) {
    ...
    for (frameidx = 0; frameidx < maxframes && outputsamples < samplecount; frameidx++) {
        ...
        if (frameidx == 0) {
            ...
            if (encodedebug) ms_log(1, "Frame %d: X0=%d\n", frameidx, frameptr[1]);

            if (swapflag) ms_gswap4a(&frameptr[1]);
            ...
            if (encodedebug) ms_log(1, "Frame %d\n", frameidx);
        }
        ...
        for (widx = startnibble; widx < 16 && outputsamples < samplecount; widx++) {
            ...
            /* 2 x 16-bit differences */
                ...
                if (encodedebug) ms_log(1, "  W%02d: 2=2x16b  %d  %d\n", widx, diffs[0], diffs[1]);
            ...
            /* 1 x 32-bit difference */
                ...
                if (encodedebug) ms_log(1, "  W%02d: 3=1x32b  %d\n", widx, diffs[0]);

                frameptr[widx] = diffs[0];

                if (swapflag) ms_gswap4a(&frameptr[widx]);
        ...
        if (swapflag) ms_gswap4a(&frameptr[0]);
    } /* Done with frames */

    /* Set Xn (reverse integration constant) in first frame to last sample */
    if (Xnp) *Xnp = *(input + outputsamples - 1);
    if (swapflag) ms_gswap4a(Xnp);

    /* Pad any remaining bytes */
    if ((frameidx * 64) < outputlength) memset(output + (frameidx * 16), 0, outputlength - (frameidx * 64));
 ...
 * Return number of samples in output buffer on success, -1 on failure.
 ************************************************************************/
int msr_encode_steim2(int32_t *input, int samplecount, int32_t *output, int outputlength, int32_t diff0,
    ...
    for (frameidx = 0; frameidx < maxframes && outputsamples < samplecount; frameidx++) {
        ...
        for (widx = startnibble; widx < 16 && outputsamples < samplecount; widx++) {
            ...
            /* 2 x 15-bit differences */
                ...
                if (encodedebug) ms_log(1, "  W%02d: 10,10=2x15b  %d  %d\n", widx, diffs[0], diffs[1]);
            ...
            /* 1 x 30-bit difference */
                ...
                if (encodedebug) ms_log(1, "  W%02d: 10,01=1x30b  %d\n", widx, diffs[0]);
            ...
            if (swapflag && packedsamples != 4) ms_gswap4a(&frameptr[widx]);
