...
MSTrace *mst_init(MSTrace *mst) {
    ...
    if (mst) {
        if (mst->datasamples) free(mst->datasamples);

        if (mst->prvtptr) free(mst->prvtptr);

        if (mst->ststate) free(mst->ststate);
    } else {
...
void mst_free(MSTrace **ppmst) {
    if (ppmst && *ppmst) {
        ...
        if ((*ppmst)->datasamples) free((*ppmst)->datasamples);

        /* Free private memory if present */
        if ((*ppmst)->prvtptr) free((*ppmst)->prvtptr);

        /* Free stream processing state if present */
        if ((*ppmst)->ststate) free((*ppmst)->ststate);
 ...
 ***************************************************************************/
MSTrace *mst_findmatch(MSTrace *startmst, char dataquality, char *network, char *station, char *location,
                       char *channel) {
    int idx;

    if (!startmst) return 0;
 ...
 ***************************************************************************/
MSTrace *mst_findadjacent(MSTraceGroup *mstg, flag *whence, char dataquality, char *network, char *station,
                          char *location, char *channel, double samprate, double sampratetol,
...
int mst_addmsr(MSTrace *mst, MSRecord *msr, flag whence) {
    ...
    if (whence == 1) {
        if (msr->datasamples && msr->numsamples >= 0) {
            memcpy((char *)mst->datasamples + (mst->numsamples * samplesize), msr->datasamples,
    ...
    /* Add samples at the beginning of trace */
        ...
        if (msr->datasamples && msr->numsamples >= 0) {
            ...
            if (mst->numsamples > 0) {
                memmove((char *)mst->datasamples + (msr->numsamples * samplesize), mst->datasamples,
                        (size_t)(mst->numsamples * samplesize));
            }

            memcpy(mst->datasamples, msr->datasamples, (size_t)(msr->numsamples * samplesize));
    ...
    if (mst->dataquality && msr->dataquality && mst->dataquality != msr->dataquality) mst->dataquality = 0;
 ...
 * Return 0 on success and -1 on error.
 ***************************************************************************/
int mst_addspan(MSTrace *mst, hptime_t starttime, hptime_t endtime, void *datasamples, int64_t numsamples,
    ...
    if (datasamples && numsamples > 0) {
        ...
        mst->datasamples =
                realloc(mst->datasamples, (size_t)(mst->numsamples * samplesize + numsamples * samplesize));
    ...
    if (whence == 1) {
        if (datasamples && numsamples > 0) {
            memcpy((char *)mst->datasamples + (mst->numsamples * samplesize), datasamples,
    ...
    /* Add samples at the beginning of trace */
        ...
        if (datasamples && numsamples > 0) {
            /* Move any samples to end of buffer */
            if (mst->numsamples > 0) {
                memmove((char *)mst->datasamples + (numsamples * samplesize), mst->datasamples,
                        (size_t)(mst->numsamples * samplesize));
            }

            memcpy(mst->datasamples, datasamples, (size_t)(numsamples * samplesize));
    ...
    /* Update MSTrace sample count */
    if (numsamples > 0) mst->samplecnt += numsamples;
 ...
 ***************************************************************************/
MSTrace *mst_addmsrtogroup(MSTraceGroup *mstg, MSRecord *msr, flag dataquality, double timetol,
                           double sampratetol) {
    MSTrace *mst = 0;
    hptime_t endtime;
    flag whence;
    ...
    mst = mst_findadjacent(mstg, &whence, dq, msr->network, msr->station, msr->location, msr->channel,
                           msr->samprate, sampratetol, msr->starttime, endtime, timetol);
    ...
    if (mst) {
        ...
        if (msr->samplecnt <= 0 || msr->samprate <= 0.0) return mst;
        ...
        if (!mstg->traces) {
            ...
            while (lasttrace->next) lasttrace = lasttrace->next;
...
int mst_groupheal(MSTraceGroup *mstg, double timetol, double sampratetol) {
    ...
    if (mst_groupsort(mstg, 1)) return -1;
    ...
    while (curtrace) {
        ...
        while (nexttrace) {
            ...
            if (timetol == -1.0) timetol = 0.5 * delta;

            /* Fits right at the end of curtrace */
            if (ms_dabs(postgap) <= timetol) {
                /* Merge searchtrace with curtrace */
                mst_addspan(curtrace, searchtrace->starttime, searchtrace->endtime, searchtrace->datasamples,
                            searchtrace->numsamples, searchtrace->sampletype, 1);

                /* If no data is present, make sure sample count is updated */
                if (searchtrace->numsamples <= 0) curtrace->samplecnt += searchtrace->samplecnt;

                /* If qualities do not match reset the indicator */
                if (curtrace->dataquality != searchtrace->dataquality) curtrace->dataquality = 0;
            ...
            /* Fits right at the beginning of curtrace */
                ...
                /* Merge searchtrace with curtrace */
                mst_addspan(curtrace, searchtrace->starttime, searchtrace->endtime, searchtrace->datasamples,
                            searchtrace->numsamples, searchtrace->sampletype, 2);
        ...
        while (p) {
            ...
            while (psize > 0 || (qsize > 0 && q)) {
                ...
                if (psize == 0) { /* p is empty; e must come from q. */
                    ...
                    p = p->next;
                    psize--;
                } else if (mst_groupsort_cmp(p, q, quality) <=
                           0) { /* First element of p is lower (or same), e must come from p. */
                    e = p;
                    p = p->next;
                    psize--;
    ...
    if (strcmpval > 0) {
        ...
        if (mst1->starttime > mst2->starttime) {
            ...
            if (mst1->endtime < mst2->endtime) {
                ...
                if (!MS_ISRATETOLERABLE(mst1->samprate, mst2->samprate) && mst1->samprate > mst2->samprate) {
                    return 1;
                }
            }
        }
...
int mst_convertsamples(MSTrace *mst, char type, flag truncate) {
    ...
    if (mst->sampletype == type) return 0;
        ...
        if (mst->sampletype == 'f') /* Convert floats to integers with simple rounding */
        {
            for (idx = 0; idx < mst->numsamples; idx++) {
                ...
                if (!truncate && (fdata[idx] - (int32_t)fdata[idx]) > 0.000001) {
                    ms_log(1,
                           "mst_convertsamples: Warning, loss of precision when converting floats to "
                           "integers, loss: %g\n",
        ...
        } else if (mst->sampletype == 'd') /* Convert doubles to integers with simple rounding */
        {
            for (idx = 0; idx < mst->numsamples; idx++) {
                ...
                if (!truncate && (ddata[idx] - (int32_t)ddata[idx]) > 0.000001) {
                    ms_log(1,
                           "mst_convertsamples: Warning, loss of precision when converting doubles to "
                           "integers, loss: %g\n",
            ...
            if (!(mst->datasamples =
                          realloc(mst->datasamples, (size_t)(mst->numsamples * sizeof(int32_t))))) {
    ...
    /* Convert to 32-bit floats */
        ...
        {
            for (idx = 0; idx < mst->numsamples; idx++) fdata[idx] = (float)idata[idx];
        } else if (mst->sampletype == 'd') /* Convert doubles to floats */
        {
            for (idx = 0; idx < mst->numsamples; idx++) fdata[idx] = (float)ddata[idx];
    ...
    /* Convert to 64-bit doubles */
        ...
        {
            for (idx = 0; idx < mst->numsamples; idx++) ddata[idx] = (double)idata[idx];

            free(idata);
        } else if (mst->sampletype == 'f') /* Convert floats to doubles */
        {
            for (idx = 0; idx < mst->numsamples; idx++) ddata[idx] = (double)fdata[idx];
...
void mst_printsynclist(MSTraceGroup *mstg, char *dccid, flag subsecond) {
    ...
    while (mst) {
        ...
        ms_log(0, "%s|%s|%s|%s|%s|%s||%.10g|%" PRId64 "|||||||%s\n", mst->network, mst->station,
               mst->location, mst->channel, stime, etime, mst->samprate, mst->samplecnt, yearday);
    ...
    while (mst->next) {
        ...
        if (!strcmp(src1, src2)) {
            ...
            if (gap < 0.0) {
                ...
                if ((gap * -1.0) >
                    (((double)(mst->next->endtime - mst->next->starttime) / HPTMODULUS) + delta))
            ...
            if (mingap)
                if (gap < *mingap) printflag = 0;

            if (maxgap)
                if (gap > *maxgap) printflag = 0;
 ...
 ***************************************************************************/
int mst_pack(MSTrace *mst, void (*record_handler)(char *, int, void *), void *handlerdata, int reclen,
    ...
    if (trpackedsamples > 0) {
        ...
        if (bufsize) {
            memmove(mst->datasamples, (char *)mst->datasamples + (trpackedsamples * samplesize),
                    (size_t)bufsize);
    ...
    if (packedsamples) *packedsamples = trpackedsamples;
 ...
 * Returns the number of records created on success and -1 on error.
 ***************************************************************************/
int mst_packgroup(MSTraceGroup *mstg, void (*record_handler)(char *, int, void *), void *handlerdata,
                  int reclen, flag encoding, flag byteorder, int64_t *packedsamples, flag flush, flag verbose,
                  MSRecord *mstemplate) {
    MSTrace *mst;
    int trpackedrecords = 0;
    int64_t trpackedsamples = 0;
    ...
    while (mst) {
        if (mst->numsamples <= 0) {
            ...
            trpackedrecords += mst_pack(mst, record_handler, handlerdata, reclen, encoding, byteorder,
                                        &trpackedsamples, flush, verbose, mstemplate);

            if (trpackedrecords == -1) break;
