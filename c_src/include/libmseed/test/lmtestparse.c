...
int main(int argc, char **argv) {
    ...
    if (tracegap) mstl = mstl_init(NULL);

    /* Loop over the input file */
    while ((retcode = ms_readmsr(&msr, inputfile, reclen, NULL, NULL, 1, printdata, verbose)) == MS_NOERROR) {
        totalrecs++;
    ...
    if (tracegap) mstl_printtracelist(mstl, 0, 1, 1);

    /* Make sure everything is cleaned up */
    ms_readmsr(&msr, NULL, 0, NULL, NULL, 0, 0, 0);

    if (mstl) mstl_free(&mstl, 0);

    if (basicsum) ms_log(1, "Records: %" PRId64 ", Samples: %" PRId64 "\n", totalrecs, totalsamps);
