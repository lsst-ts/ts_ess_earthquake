...
char *ms_recsrcname(char *record, char *srcname, flag quality) {
    ...
    if (quality) sprintf(srcname, "%s_%s_%s_%s_%c", network, station, location, channel, fsdh->dataquality);
...
hptime_t ms_btime2hptime(BTime *btime) {
    ...
    hptime = (hptime_t)(60 * (60 * ((hptime_t)24 * days + btime->hour) + btime->min) + btime->sec) *
                     HPTMODULUS +
...
char *ms_btime2isotimestr(BTime *btime, char *isotimestr) {
    ...
    ret = snprintf(isotimestr, 25, "%4d-%02d-%02dT%02d:%02d:%02d.%04d", btime->year, month, mday, btime->hour,
                   btime->min, btime->sec, btime->fract);
...
char *ms_btime2mdtimestr(BTime *btime, char *mdtimestr) {
    ...
    ret = snprintf(mdtimestr, 25, "%4d-%02d-%02d %02d:%02d:%02d.%04d", btime->year, month, mday, btime->hour,
                   btime->min, btime->sec, btime->fract);
...
char *ms_btime2seedtimestr(BTime *btime, char *seedtimestr) {
    ...
    if (!seedtimestr) return NULL;

    ret = snprintf(seedtimestr, 23, "%4d,%03d,%02d:%02d:%02d.%04d", btime->year, btime->day, btime->hour,
                   btime->min, btime->sec, btime->fract);
 ...
 * Returns 0 on success and -1 on error.
 ***************************************************************************/
int ms_hptime2tomsusecoffset(hptime_t hptime, hptime_t *toms, int8_t *usecoffset) {
    if (toms == NULL || usecoffset == NULL) return -1;
...
int ms_hptime2btime(hptime_t hptime, BTime *btime) {
    ...
    if (hptime < 0 && ifract != 0) {
        ...
        if (ifract - bfract * (HPTMODULUS / 10000)) bfract -= 1;

        isec -= 1;
        bfract = 10000 - (-bfract);
    }

    if (!(ms_gmtime_r(&isec, &tms))) return -1;
 ...
 * Returns a pointer to the resulting string or NULL on error.
 ***************************************************************************/
char *ms_hptime2isotimestr(hptime_t hptime, char *isotimestr, flag subseconds) {
    struct tm tms;
    int64_t isec;
    int ifract;
    int ret;

    if (isotimestr == NULL) return NULL;
    ...
    if (!(ms_gmtime_r(&isec, &tms))) return NULL;

    if (subseconds) /* Assuming ifract has at least microsecond precision */
        ret = snprintf(isotimestr, 27, "%4d-%02d-%02dT%02d:%02d:%02d.%06d", tms.tm_year + 1900,
                       tms.tm_mon + 1, tms.tm_mday, tms.tm_hour, tms.tm_min, tms.tm_sec, ifract);
    else
        ret = snprintf(isotimestr, 20, "%4d-%02d-%02dT%02d:%02d:%02d", tms.tm_year + 1900, tms.tm_mon + 1,
...
char *ms_hptime2mdtimestr(hptime_t hptime, char *mdtimestr, flag subseconds) {
    ...
    if (subseconds) /* Assuming ifract has at least microsecond precision */
        ret = snprintf(mdtimestr, 27, "%4d-%02d-%02d %02d:%02d:%02d.%06d", tms.tm_year + 1900, tms.tm_mon + 1,
                       tms.tm_mday, tms.tm_hour, tms.tm_min, tms.tm_sec, ifract);
    else
        ret = snprintf(mdtimestr, 20, "%4d-%02d-%02d %02d:%02d:%02d", tms.tm_year + 1900, tms.tm_mon + 1,
                       tms.tm_mday, tms.tm_hour, tms.tm_min, tms.tm_sec);
 ...
 * Returns a pointer to the resulting string or NULL on error.
 ***************************************************************************/
char *ms_hptime2seedtimestr(hptime_t hptime, char *seedtimestr, flag subseconds) {
    struct tm tms;
    int64_t isec;
    int ifract;
    int ret;

    if (seedtimestr == NULL) return NULL;
    ...
    if (subseconds) /* Assuming ifract has at least microsecond precision */
        ret = snprintf(seedtimestr, 25, "%4d,%03d,%02d:%02d:%02d.%06d", tms.tm_year + 1900, tms.tm_yday + 1,
                       tms.tm_hour, tms.tm_min, tms.tm_sec, ifract);
    else
        ret = snprintf(seedtimestr, 18, "%4d,%03d,%02d:%02d:%02d", tms.tm_year + 1900, tms.tm_yday + 1,
...
hptime_t ms_seedtimestr2hptime(char *seedtimestr) {
    ...
    fields = sscanf(seedtimestr, "%d%*[-,:.]%d%*[-,:.Tt ]%d%*[-,:.]%d%*[-,:.]%d%f", &year, &day, &hour, &min,
                    &sec, &fusec);
...
hptime_t ms_timestr2hptime(char *timestr) {
    ...
    fields = sscanf(timestr, "%d%*[-,/:.]%d%*[-,/:.]%d%*[-,/:.Tt ]%d%*[-,/:.]%d%*[-,/:.]%d%f", &year, &mon,
                    &mday, &hour, &min, &sec, &fusec);
 ...
 * Returns 0 on success and -1 on error.
 ***************************************************************************/
int ms_reduce_rate(double samprate, int16_t *factor1, int16_t *factor2) {
    int num;
    int den;
...
int ms_genfactmult(double samprate, int16_t *factor, int16_t *multiplier) {
    ...
    if (!factor || !multiplier) return -1;

    /* Convert sample period to sample rate */
    if (samprate < 0.0) samprate = -1.0 / samprate;
