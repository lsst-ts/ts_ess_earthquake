...
MSRecord *msr_init(MSRecord *msr) {
    ...
    if (!msr) {
        ...
        if (msr->blkts) msr_free_blktchain(msr);

        if (msr->ststate) free(msr->ststate);
    }
...
void msr_free(MSRecord **ppmsr) {
    if (ppmsr != NULL && *ppmsr != 0) {
        ...
        if ((*ppmsr)->fsdh) free((*ppmsr)->fsdh);

        /* Free blockette chain if populated */
        if ((*ppmsr)->blkts) msr_free_blktchain(*ppmsr);

        /* Free datasamples if present */
        if ((*ppmsr)->datasamples) free((*ppmsr)->datasamples);

        /* Free stream processing state if present */
        if ((*ppmsr)->ststate) free((*ppmsr)->ststate);
    ...
    if (msr) {
        if (msr->blkts) {
            ...
            while (bc) {
                ...
                if (bc->blktdata) free(bc->blktdata);
...
int msr_normalize_header(MSRecord *msr, flag verbose) {
    ...
    if (msr->fsdh) {
        if (verbose > 2) ms_log(1, "Normalizing fixed section of data header\n");
    ...
    if (msr->fsdh) msr->fsdh->numblockettes = blktcnt;
...
MSRecord *msr_duplicate(MSRecord *msr, flag datadup) {
    ...
    if ((dupmsr = msr_init(NULL)) == NULL) return NULL;
...
hptime_t msr_starttime(MSRecord *msr) {
    ...
    if (!msr || starttime == HPTERROR) return HPTERROR;
...
hptime_t msr_starttime_uc(MSRecord *msr) {
    if (!msr) return HPTERROR;

    if (!msr->fsdh) return HPTERROR;
...
hptime_t msr_endtime(MSRecord *msr) {
    ...
    if (lslist) {
        while (lslist) {
            if (lslist->leapsecond > msr->starttime && lslist->leapsecond < (msr->starttime + span)) {
                span -= HPTMODULUS;
                break;
            }
        ...
        if (msr->fsdh)
            if (msr->fsdh->act_flags & 0x10) span -= HPTMODULUS;
    }
...
void msr_print(MSRecord *msr, flag details) {
    ...
    if (details > 0 && msr->fsdh) {
        ...
        ms_log(0, "%s, %06d, %c, %d, %" PRId64 " samples, %-.10g Hz, %s\n", srcname, msr->sequence_number,
               msr->dataquality, msr->reclen, msr->samplecnt, msr->samprate, time);
    }
...
double msr_host_latency(MSRecord *msr) {
    ...
    if (msr == NULL) return 0.0;

    /* Calculate the time covered by the samples */
    if (msr->samprate > 0.0 && msr->samplecnt > 0) span = (1.0 / msr->samprate) * (msr->samplecnt - 1);
