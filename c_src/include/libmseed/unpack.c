...
int msr_unpack(char *record, int reclen, MSRecord **ppmsr, flag dataflag, flag verbose) {
    ...
    if (!(*ppmsr = msr_init(*ppmsr))) return MS_GENERROR;
    ...
    if (unpackheaderbyteorder == -2 || unpackdatabyteorder == -2 || unpackencodingformat == -2 ||
        unpackencodingfallback == -2)
        if (check_environment(verbose)) return MS_GENERROR;
    ...
    while ((blkt_offset != 0) && ((int)blkt_offset < reclen) && (blkt_offset < MAXRECLEN)) {
        ...
        if (blkt_type == 100) { /* Found a Blockette 100 */
            struct blkt_100_s *blkt_100;

            blkt_link = msr_addblockette(msr, record + blkt_offset, sizeof(struct blkt_100_s), blkt_type, 0);
            if (!blkt_link) break;
        ...
        else if (blkt_type == 200) { /* Found a Blockette 200 */
            struct blkt_200_s *blkt_200;

            blkt_link = msr_addblockette(msr, record + blkt_offset, sizeof(struct blkt_200_s), blkt_type, 0);
            if (!blkt_link) break;
        ...
        else if (blkt_type == 201) { /* Found a Blockette 201 */
            struct blkt_201_s *blkt_201;

            blkt_link = msr_addblockette(msr, record + blkt_offset, sizeof(struct blkt_201_s), blkt_type, 0);
            if (!blkt_link) break;
        ...
        else if (blkt_type == 300) { /* Found a Blockette 300 */
            struct blkt_300_s *blkt_300;

            blkt_link = msr_addblockette(msr, record + blkt_offset, sizeof(struct blkt_300_s), blkt_type, 0);
            if (!blkt_link) break;
        ...
        else if (blkt_type == 310) { /* Found a Blockette 310 */
            struct blkt_310_s *blkt_310;

            blkt_link = msr_addblockette(msr, record + blkt_offset, sizeof(struct blkt_310_s), blkt_type, 0);
            if (!blkt_link) break;
        ...
        else if (blkt_type == 320) { /* Found a Blockette 320 */
            struct blkt_320_s *blkt_320;

            blkt_link = msr_addblockette(msr, record + blkt_offset, sizeof(struct blkt_320_s), blkt_type, 0);
            if (!blkt_link) break;
        ...
        else if (blkt_type == 390) { /* Found a Blockette 390 */
            struct blkt_390_s *blkt_390;

            blkt_link = msr_addblockette(msr, record + blkt_offset, sizeof(struct blkt_390_s), blkt_type, 0);
            if (!blkt_link) break;
        ...
        else if (blkt_type == 395) { /* Found a Blockette 395 */
            struct blkt_395_s *blkt_395;

            blkt_link = msr_addblockette(msr, record + blkt_offset, sizeof(struct blkt_395_s), blkt_type, 0);
            if (!blkt_link) break;
        ...
        else if (blkt_type == 400) { /* Found a Blockette 400 */
            struct blkt_400_s *blkt_400;

            blkt_link = msr_addblockette(msr, record + blkt_offset, sizeof(struct blkt_400_s), blkt_type, 0);
            if (!blkt_link) break;
        ...
        else if (blkt_type == 405) { /* Found a Blockette 405 */
            struct blkt_405_s *blkt_405;

            blkt_link = msr_addblockette(msr, record + blkt_offset, sizeof(struct blkt_405_s), blkt_type, 0);
            if (!blkt_link) break;
        ...
        else if (blkt_type == 500) { /* Found a Blockette 500 */
            struct blkt_500_s *blkt_500;

            blkt_link = msr_addblockette(msr, record + blkt_offset, sizeof(struct blkt_500_s), blkt_type, 0);
            if (!blkt_link) break;
        ...
        else if (blkt_type == 1000) { /* Found a Blockette 1000 */
            struct blkt_1000_s *blkt_1000;

            blkt_link = msr_addblockette(msr, record + blkt_offset, sizeof(struct blkt_1000_s), blkt_type, 0);
            if (!blkt_link) break;
        ...
        else if (blkt_type == 1001) { /* Found a Blockette 1001 */
            blkt_link = msr_addblockette(msr, record + blkt_offset, sizeof(struct blkt_1001_s), blkt_type, 0);
            if (!blkt_link) break;
        ...
        else if (blkt_type == 2000) { /* Found a Blockette 2000 */
        ...
        if (next_blkt && next_blkt < (blkt_offset + blkt_length - 4)) {
            ms_log(2,
                   "msr_unpack(%s): Offset to next blockette (%d) is within current blockette ending at byte "
                   "%d\n",
    ...
    if (blkt_link && msr->fsdh->numsamples &&
        msr->fsdh->data_offset < (blkt_link->blktoffset + blkt_link->blktdatalen + 4)) {
        ms_log(1,
               "%s: Warning: Data offset in fixed header (%d) is within the blockette chain ending at %d\n",
    ...
    if (msr->fsdh->numblockettes != blkt_count) {
        ms_log(1,
               "%s: Warning: Number of blockettes in fixed header (%d) does not match the number parsed "
               "(%d)\n",
    ...
    if (dataflag && msr->samplecnt > 0) {
        ...
        if (msr->Blkt1000 != 0 && unpackdatabyteorder < 0) {
            ...
            if (bigendianhost && msr->byteorder == 0) dswapflag = 1;
        ...
        if (msr->datasamples) free(msr->datasamples);
...
int msr_unpack_data(MSRecord *msr, int swapflag, flag verbose) {
    ...
    if (!msr) return MS_GENERROR;

    /* Check for decode debugging environment variable */
    if (getenv("DECODE_DEBUG")) decodedebug = 1;
    ...
    if (verbose > 2) ms_log(1, "%s: Unpacking %" PRId64 " samples\n", srcname, msr->samplecnt);

    /* Decode data samples according to encoding */
    switch (msr->encoding) {
        case DE_ASCII:
            if (verbose > 1) ms_log(1, "%s: Found ASCII data\n", srcname);
        ...
        case DE_INT16:
            if (verbose > 1) ms_log(1, "%s: Unpacking INT16 data samples\n", srcname);

            nsamples = msr_decode_int16((int16_t *)dbuf, (int)msr->samplecnt, msr->datasamples, unpacksize,
                                        swapflag);
        ...
        case DE_INT32:
            if (verbose > 1) ms_log(1, "%s: Unpacking INT32 data samples\n", srcname);

            nsamples = msr_decode_int32((int32_t *)dbuf, (int)msr->samplecnt, msr->datasamples, unpacksize,
                                        swapflag);
        ...
        case DE_FLOAT32:
            if (verbose > 1) ms_log(1, "%s: Unpacking FLOAT32 data samples\n", srcname);

            nsamples = msr_decode_float32((float *)dbuf, (int)msr->samplecnt, msr->datasamples, unpacksize,
                                          swapflag);
        ...
        case DE_FLOAT64:
            if (verbose > 1) ms_log(1, "%s: Unpacking FLOAT64 data samples\n", srcname);

            nsamples = msr_decode_float64((double *)dbuf, (int)msr->samplecnt, msr->datasamples, unpacksize,
                                          swapflag);
        ...
        case DE_STEIM1:
            if (verbose > 1) ms_log(1, "%s: Unpacking Steim1 data frames\n", srcname);

            nsamples = msr_decode_steim1((int32_t *)dbuf, datasize, (int)msr->samplecnt, msr->datasamples,
                                         unpacksize, srcname, swapflag);

            if (nsamples < 0) return MS_GENERROR;

            msr->sampletype = 'i';
            break;

        case DE_STEIM2:
            if (verbose > 1) ms_log(1, "%s: Unpacking Steim2 data frames\n", srcname);

            nsamples = msr_decode_steim2((int32_t *)dbuf, datasize, (int)msr->samplecnt, msr->datasamples,
                                         unpacksize, srcname, swapflag);
        ...
        case DE_GEOSCOPE164:
            ...
            nsamples = msr_decode_geoscope((char *)dbuf, (int)msr->samplecnt, msr->datasamples, unpacksize,
                                           msr->encoding, srcname, swapflag);

            msr->sampletype = 'f';
            break;

        case DE_CDSN:
            if (verbose > 1) ms_log(1, "%s: Unpacking CDSN encoded data samples\n", srcname);

            nsamples = msr_decode_cdsn((int16_t *)dbuf, (int)msr->samplecnt, msr->datasamples, unpacksize,
                                       swapflag);
        ...
        case DE_SRO:
            if (verbose > 1) ms_log(1, "%s: Unpacking SRO encoded data samples\n", srcname);

            nsamples = msr_decode_sro((int16_t *)dbuf, (int)msr->samplecnt, msr->datasamples, unpacksize,
                                      srcname, swapflag);
        ...
        case DE_DWWSSN:
            if (verbose > 1) ms_log(1, "%s: Unpacking DWWSSN encoded data samples\n", srcname);

            nsamples = msr_decode_dwwssn((int16_t *)dbuf, (int)msr->samplecnt, msr->datasamples, unpacksize,
                                         swapflag);
        ...
        default:
            ms_log(2, "%s: Unsupported encoding format %d (%s)\n", srcname, msr->encoding,
                   (char *)ms_encodingstr(msr->encoding));
    ...
    if (unpackheaderbyteorder == -2) {
        if ((envvariable = getenv("UNPACK_HEADER_BYTEORDER"))) {
            if (*envvariable != '0' && *envvariable != '1') {
                ...
                if (verbose > 2) ms_log(1, "UNPACK_HEADER_BYTEORDER=0, unpacking little-endian header\n");
            } else {
                unpackheaderbyteorder = 1;
                if (verbose > 2) ms_log(1, "UNPACK_HEADER_BYTEORDER=1, unpacking big-endian header\n");
            }
        } else {
            unpackheaderbyteorder = -1;
        }
    ...
    if (unpackdatabyteorder == -2) {
        if ((envvariable = getenv("UNPACK_DATA_BYTEORDER"))) {
            if (*envvariable != '0' && *envvariable != '1') {
                ...
                if (verbose > 2) ms_log(1, "UNPACK_DATA_BYTEORDER=0, unpacking little-endian data samples\n");
            } else {
                unpackdatabyteorder = 1;
                if (verbose > 2) ms_log(1, "UNPACK_DATA_BYTEORDER=1, unpacking big-endian data samples\n");
            }
        } else {
            unpackdatabyteorder = -1;
        }
